...
Structure mirrors the contract: one helper per top-level block, so a v1 minor
re-vendor only touches the helper for the block that changed.
...
def _validate_producer(producer, v):
    if producer is None:
        return
...
def _validate_observation(observation, v):
...
def _validate_allof(obj, v):
    ...
    producer, run = obj.get("producer"), obj.get("run")
...
def validate_obj(obj):
    ...
    v = []
    _validate_top(obj, v)
    _validate_producer(obj.get("producer"), v)
    _validate_experiment(obj.get("experiment"), v)
    _validate_run(obj.get("run"), v)
    _validate_observation(obj.get("observation"), v)
    _validate_allof(obj, v)
    return v
